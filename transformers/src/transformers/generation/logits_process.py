...
class ACSLogitsWarper(LogitsProcessor):
    """
    Implementation of "Adaptive Contrastive Search" (ACS) paper: https://arxiv.org/abs/2407.18698v2 
    Args:
        pad_token_id: The pad token id to mask out before we apply the degeneration penalty
        q: The parameter for the dynamic adjustment of "temperature" proposed in the paper
        penalty: The parameter for the degeneration penalty
    """
    def __init__(
        self, 
        pad_token_id: int,
    ...
    def _degeneration_penalty(self, 
                            ...
                            alpha_t: float) -> torch.FloatTensor:
        """
        Apply degeneration penalty as described in the paper
        """
        ...
        for b in range(batch_size):
            ...
            # Apply penalty based on token frequency
            penalties = torch.pow(self.penalty, token_counts * alpha_t)
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        """
        Apply ACS to modify token probabilities
        """
        epsilon = 1e-5
        # Calculate current step entropy
        probs = torch.softmax(scores, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs + epsilon))
        self.steps_entropy.append(entropy)
        
        # Skip parameter adjustment if we don't have enough steps
        ...
        # Apply degeneration penalty to remaining tokens
        adjusted_probs = self._degeneration_penalty(scores, input_ids, alpha_t)
...
class DynamicLogitsWarper(LogitsProcessor):
    ...
    def _dynamic_adjustment(self, 
        ...
        if self.time_step < self.window_size:
            ...
            delta_t_raw = (step_entropy[-1] - median_local) / max_entropy
            
            # Global entropy calculation - compare window median to global statistics
            window_median = torch.median(step_entropy[-self.window_size:])
            delta_global_raw = (window_median - global_entropy) / max_entropy
        ...
        # print(delta_t_raw, "this is delta_t_raw")
        # print(delta_global_raw, "this is delta_global_raw")

        # Calculate deltas
        delta_t = self.q * torch.atanh(torch.clamp(delta_t_raw, -1 + epsilon, 1 - epsilon))
        delta_global = self.q * torch.atanh(torch.clamp(delta_global_raw, -1 + epsilon, 1 - epsilon))
        # print(delta_t, "this is delta_t")
        # print(delta_global, "this is delta_global")
        ...
        k_t = 10 * exp_value / (1 + exp_value) + 5  # Added +5 to ensure minimum k value
        # print(k_t, "this is the value of k_t")
        if torch.isnan(k_t):
            k_t = torch.tensor(10, device=step_entropy.device)
        ...
        if self.time_step < self.window_size:
            ...
            delta_loc_a = (step_entropy[-1] - median_local) / ln_k
            delta_glob_a = (window_median - global_entropy) / ln_k
    ...
    def _degeneration_penalty(self, 
                            ...
                            alpha_t: float) -> torch.FloatTensor:
        """
        Apply degeneration penalty using probability-based approach
        """
        ...
        adjusted_probs = torch.zeros_like(probs)
        for b in range(batch_size):
            sequence_tokens = input_ids[b]
            sequence_tokens = sequence_tokens[pad_mask[b] == 1]
            token_counts = torch.bincount(sequence_tokens, minlength=vocab_size).to(device=scores.device)
            # Calculate penalties
            penalties = torch.pow(alpha_t, token_counts)
            
            # Apply penalties to probabilities
            adjusted_probs[b] = probs[b] * penalties
            
            # Normalize
            adjusted_probs[b] = adjusted_probs[b] / torch.sum(adjusted_probs[b], dim=-1, keepdim=True)

        return adjusted_probs

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # Calculate probabilities and entropy
        probabilities = torch.softmax(scores / self.temperature, dim=-1)
        ...
        # Apply top-k sampling with k from dynamic adjustment
        top_k = int(k_t)
        # print(top_k, "this is top_k")
        ...
        # Create final mask
        # mask = torch.ones_like(scores) * self.filter_value
        # mask.scatter_(-1, top_k_indices, adjusted_probs)
